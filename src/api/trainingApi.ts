...
function applyFilters(
  ...
  return lessons.filter((lesson) => {
    ...
    if (
      ...
      filters.instructorName !== "All" &&
      lesson.instructorName !== filters.instructorName
    )
      return false;
    if (
      filters.lessonStatus &&
      filters.lessonStatus !== "All" &&
      lesson.lessonStatus !== filters.lessonStatus
    )
      return false;
    if (filters.traineeSearch) {
      const search = filters.traineeSearch.trim().toLowerCase();
      if (search && !lesson.traineeName.toLowerCase().includes(search))
        return false;
    }
    return true;
  });
 ...
 */
export async function getTrainingLessons(
  filters?: LessonFilters,
